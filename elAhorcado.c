...
void game(struct Player *P){
	...
	int i=10;
    	while(i>0){
		//Comprueba si el array con las letras introducidas es igual a la palabra secreta
		int compareLetras = strcmp(adivina, sec); 
		if(compareLetras==0){
			P->points+=10;
		...
		scanf("%s", &letra);
		int compareWord = strcmp(letra,sec); //compara lo introducido con la palabra misteriosa
		//esta funcion es necesaria porque el usuario puede apuntar la solución en una vez.
		if(compareWord==0){
			P->points+=10;
			printf("\n              *** ¡Has adivinado el misterio! ***\n");
			printf("\n                 La palabra misteriosa es %s.\n\n",sec);
		...
		if(i>2){
			printf("\n            Te quedan %d intentos con %d fallos.\n", i-1, (7-P->failed));	
		}
		fflush(stdin);
	i--;
	}
